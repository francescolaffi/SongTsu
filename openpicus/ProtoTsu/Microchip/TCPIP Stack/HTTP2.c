...
static void HTTPProcess(void)
{
    ...
    do
    {
		...
		case SM_HTTP_PARSE_REQUEST:
            ...
            // Determine if GET, or Websocket request
			#ifdef STACK_USE_WEBSOCKETS
			int KeyPosition;
			//IsUpgrade = TCPFindArrayEx(sktHTTP, (BYTE*)"Upgrade: websocket", 18, 0, HTTP_MAX_DATA_LEN, TRUE);
			// if (IsUpgrade != 0xFFFF) {//search array found
			KeyPosition = TCPFindArrayEx(sktHTTP, (BYTE*) "Sec-WebSocket-Key: ", 19, 0, HTTP_MAX_DATA_LEN, TRUE);
			if (KeyPosition != 0xFFFF) {
				TCPGetArray(sktHTTP, curHTTP.data, 450);
				smHTTP = SM_HTTP_INIT_WEBSOCKET;
				curHTTP.KeyPosition = KeyPosition + 19;
				curHTTP.httpStatus = WEBSOCKET_FRAME;
				//TCPAdjustFIFOSize(sktHTTP, 1, 0, TCP_ADJUST_GIVE_REST_TO_TX);
				TCPAdjustFIFOSize(sktHTTP, 250, 250, 0);
				break;
            }
			// }
			//TCPGetArray(sktHTTP, curHTTP.data, 450);
		...
		case SM_HTTP_INIT_WEBSOCKET:
            ...
            int Test;
			while(EndKey < 40) {
				Test = curHTTP.data[curHTTP.KeyPosition + EndKey];
				if(Test == 0xA || Test == 0xD) {
					break;
				} else {
					EndKey++;
				}
			...
			CreateHandShake(sktHTTP,&curHTTP.data[curHTTP.KeyPosition],EndKey);
			//TCPFlush(sktHTTP);
			WsOnOpen(curHTTPID);
		...
		case SM_HTTP_PROC_WEBSOCKET:
			...
			if (ReplyLength && (TCPIsPutReady(sktHTTP) >= MaxFrameLength + 2)) {
				...
				//TCPGetArray(sktHTTP, (BYTE*)&MyBuff, EchoLength);
				...
				if (!MyFrame.FIN) {
					...
					TCPFlush(sktHTTP);
					#else
					WsOnText(curHTTPID, MyFrame.data, ReplyLength);
					#endif
				} else if (MyFrame.Opcode == BinaryFrame) {
					//Haven't seen anyone with this capability yet...
					WsOnBinary(curHTTPID, MyFrame.data, ReplyLength);
				...
				//JH if (WebSocketEventRequest[curHTTPID]) {
				if (WsDoSend(curHTTPID)) {
					if (TCPIsPutReady(sktHTTP) >= MaxFrameLength + 2) {
						BYTE MyOutBuff[MaxFrameLength + 2];
						int StreamLength, Opcode;
						WsReturnSendFrame(curHTTPID, MyOutBuff, &StreamLength, &Opcode);
						if (StreamLength > 0) {
							WebSocketFrame OutFrame;
							OutFrame = CreateFrame(MyOutBuff, StreamLength, Opcode);
							TCPPutArray(sktHTTP, (BYTE*) &OutFrame, StreamLength + 2);
							TCPFlush(sktHTTP);
						}
					}
					//JH WebSocketEventRequest[curHTTPID] = 0;//Clear event request
				}
			}
			break;
